...
export default function CreateActionsPlan({ diagnosisId, actionOrigin, criteriaId, criteriaDesc, entrepriseId, entreprise, onClose }) {
  ...
  const [pilotesDetails, setPilotesDetails] = useState([]);
  ...
  const filteredOptions = useMemo(() => {
    return pilotesDetails.filter(pilot =>
      pilot.lastname.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [searchTerm, pilotesDetails]);
  ...
  useEffect(() => {
    ...
    if (Object.keys(pilotesDetails).length > 0) {
  ...
  const getAgentName = (agentId) => {
    const index = pilotesDetails.findIndex(pilot => pilot.id === agentId);
    if (index !== -1) {
      const firstName = pilotesDetails[index].firstname;
      const lastName = pilotesDetails[index].lastname;
                    ...
                    <motion.div
                      ...
                      <div className='flex-1 flex-col gap-2'>
                        ...
                        <div className='overflow-y-auto flex flex-col gap-2 h-36 md:h-52'>
                          {pilotesDetails.length > 0 && searchTerm === '' ? (
                            pilotesDetails.map((pilot, index) =>
                              <div key={index}
                                onClick={() => {
                                  const newActionsDetails = [...actionsDetails];
                                  newActionsDetails[currentActionIndex].chosenAgent = pilot.id;
                                  setActionsDetails(newActionsDetails);
                                  setIsSelectionOpen(false);
                                }}
                                className={`flex gap-2 cursor-pointer hover:bg-gray-100 rounded-lg items-center p-1 ${actionsDetails[currentActionIndex].chosenAgent === pilot.id ? 'bg-gray-200' : ''}`}
                              >
                                <img src={`${appUrl}/images/${pilot.imagePath}`} className='h-6 w-6 rounded-full object-cover' alt="Votre profil" />
                                <h2 className='font-p_black text-lg'>{pilot.firstname} {pilot.lastname}</h2>
                              </div>
                            )) : searchTerm !== '' ? (
                              filteredOptions.map((pilot, index) => (
                                <div key={index}
                                  onClick={() => {
                                    const agentId = pilot.id;
                                    const newActionsDetails = [...actionsDetails];
